...
function connectWebsocket() {
    ...
    socket.onopen = function () {
        ...
        var auth = {
            ...
            api_key: API_Key,
            events: [
                "EVENT_COUNTDOWN_" + RewardNumber + "_REDEEMED",
                "EVENT_RESET_" + RewardNumber
            ]
        };
    ...
    socket.onmessage = function (message) {
        ...
        if (socketMessage.event === "EVENT_COUNTDOWN_" + RewardNumber + "_REDEEMED") {
        ...
        else if (socketMessage.event === "EVENT_RESET_" + RewardNumber) {
            data = JSON.parse(socketMessage.data);
            $("#header").html(data.title);
            if (timer <= 0) {
                PlaySound(data.redeemedSFXPath, data.redeemedSFXVolume)
